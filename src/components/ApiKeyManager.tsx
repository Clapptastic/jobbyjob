...
const log = logger('ApiKeyManager');
...
const API_PROVIDERS = {
  jobscraper: {
    name: 'Job Scraper Pro',
    url: 'http://localhost:8501',
    description: 'Required for job search and matching',
    required: true
  },
  openai: {
    name: 'OpenAI',
  ...
  affinda: {
    name: 'Affinda',
    url: 'https://api.affinda.com/auth/login',
    description: 'Optional resume parsing service',
    required: false
  }
};
...
export default function ApiKeyManager() {
  ...
  // Auto-collapse when we have all required keys
  useEffect(() => {
    const hasAllRequired = Object.entries(API_PROVIDERS)
      .filter(([_, config]) => config.required)
      .every(([provider]) => 
        apiKeys.some(key => key.provider === provider && key.is_active)
      );
  ...
  const missingRequired = Object.entries(API_PROVIDERS)
    .filter(([_, config]) => config.required)
    .filter(([provider]) => 
      !apiKeys.some(key => key.provider === provider && key.is_active)
    );
                ...
                <div className="flex items-center justify-between mb-2">
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      {provider?.required && (
                        <span className="text-xs text-neon-pink">Required</span>
                      )}
                    </div>
                    ...
                    {Object.entries(API_PROVIDERS)
                      ...
                      .map(([id, provider]) => (
                        <option key={id} value={id}>
                          {provider.name}{provider.required ? ' (Required)' : ''}
                        </option>
                      ))
                    }
                  </select>
